...
import (
	...
	"gitee.com/zhaochuninhefei/fabric-config-gm/cfssl/helpers"
...
func certIsRevokedOCSP(leaf *x509.Certificate, strict bool) (revoked, ok bool, e error) {
	...
	ocspRequest, err := ocsp.CreateRequest(leaf.ToX509Certificate(), issuer.ToX509Certificate(), &ocspOpts)
	if err != nil {
...
func sendOCSPRequest(server string, req []byte, leaf, issuer *x509.Certificate) (*ocsp.Response, error) {
	...
	return ocsp.ParseResponseForCert(body, leaf.ToX509Certificate(), issuer.ToX509Certificate())
}
